...
public:
  ...
  FindDuplicates(const std::vector<FileInfo> &files) {
    ...
    struct Key {
      ...
      [[nodiscard]] bool operator<(const Key &rhs) const noexcept {
        if (fileSize == rhs.fileSize) {
          return blockHashes < rhs.blockHashes;
        }
    ...
    auto advanceIterator = [&workMap](const auto it) {
      if (it->second.size() <= 1) {
        return workMap.erase(it);
      } else {
        return std::next(it);
      }
    };

    for (auto it = workMap.begin(); it != workMap.end();
         it = advanceIterator(it)) {
